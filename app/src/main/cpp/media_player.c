...
int decode_video(MediaPlayer *player, AVPacket *packet) {
    ...
    int frameFinished;
    //decode video frame
    int ret = avcodec_decode_video2(player->video_codec_context, player->yuv_frame, &frameFinished,
                                    packet);
    if (ret < 0) {
...
int decode_audio(MediaPlayer *player, AVPacket *packet) {
    int got_frame = 0, ret;
    ret = avcodec_decode_audio4(player->audio_codec_context, player->audio_frame, &got_frame,
                                packet);
    if (ret < 0) {
...
Java_com_swt_ffmpeglib_MediaPlayer_setup(JNIEnv *env, jobject thiz, jstring file_path,
                                         jobject surface) {
    const char *file_name = (*env)->GetStringUTFChars(env, file_path, JNI_FALSE);
    int ret;
...
JNIEXPORT jint JNICALL
Java_com_swt_ffmpeglib_MediaPlayer_play(JNIEnv *env, jobject thiz) {
...
Java_com_swt_ffmpeglib_MediaPlayer_release(JNIEnv *env, jobject thiz) {
    free(player
                 ->audio_track);
    free(player
    ...
    av_free(player
                    ->audio_frame);
    avcodec_close(player
                          ->video_codec_context);
    avcodec_close(player
                          ->audio_codec_context);
    avformat_close_input(&player->format_context);
    ANativeWindow_release(player
                                  ->native_window);
    delete_queue(player);
